...
/*   Updated: 2016/06/28 21:28:17 by Rakiah           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_vector3f	calculate_sub_pixel(t_scene *scene, t_camera *cam, t_vector2f pos, float coef)
{
	...
	while (42)
	{
		...
		if (hit.object->mat->reflection != 0 && ray.level_reflection < MAX_RAY_REFLECTIONS)
			reflection(&ray, &hit, &coef);
		else if (hit.object->mat->transparency != 0 && ray.level_refraction < MAX_RAY_REFRACTION)
								...
								float coef)
{
	...
	color = v3f_mul_float(v3f_mul(v3f_mul(hit->object->mat->diffuse,
										calculate_texture_color(hit)),
									scene->ambient_light), coef);
	while (++i < scene->lights_count)
	{
		...
		photon.length = v3f_distance(light->position, hit->point);
		if ((transparency = calculate_shadow_coef(&photon, scene)))
			color = v3f_add(color,
					v3f_mul_float(v3f_add(
						light_get_specular(light, ray, &photon, hit),
						light_get_diffuse(light, &photon, hit)), coef * transparency));
	}
	return (color);
}
