...
/*   Updated: 2016/06/28 19:10:29 by Rakiah           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		post_effects_sepia(t_texture *t)
{
	...
	while (++i < t->size / t->bytes_per_pixel)
	{
		...
		sepia.x = (color[0] * 0.393f) + (color[1] * 0.769f) + (color[2] * 0.189f);
		sepia.y = (color[0] * 0.349f) + (color[1] * 0.686f) + (color[2] * 0.168f);
		sepia.z = (color[0] * 0.272f) + (color[1] * 0.534f) + (color[2] * 0.131f);
		*((unsigned int *)(t->pixels + (i * t->bytes_per_pixel))) =
