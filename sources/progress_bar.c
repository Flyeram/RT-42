...
/*   Updated: 2016/06/28 02:10:47 by Rakiah           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	progress_bar_init(t_interface *ui, t_vector2f resolution)
{
	...
	ui->bar_bounding = element_new(ui, 0xFFFFFF, V2F_INIT((resolution.x / 2) - (remap.x / 2), (resolution.y / 2) - (remap.y / 2)), V2F_INIT(remap.x, remap.y));

	ui->bar_dynamic = element_new(ui, 0x00FF00, V2F_INIT((resolution.x / 2) - ((remap.x - (remap.x / 35)) / 2), (resolution.y / 2) - ((remap.y - (remap.y / 10)) / 2)), V2F_INIT(0, 0));
	ui->bar_dynamic_size = V2F_INIT(remap.x - (remap.x / 35), remap.y - (remap.y / 10));
